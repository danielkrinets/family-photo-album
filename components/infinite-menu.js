// Vanilla-JS port of React Bits' InfiniteMenu (WebGL2 sphere of photo discs).
// Requires vendor/gl-matrix.min.js to be loaded first (exposes window.glMatrix).
(function () {
...
`;
  ...
  function createInfiniteMenu(container, items, options = {}) {
    const { backgroundColor = '#000000', scale = 1.0, onSelect } = options;

    container.classList.add('infinite-menu');
    container.style.setProperty('--infinite-menu-background', backgroundColor);
    container.innerHTML = `
      <canvas class="infinite-menu-canvas"></canvas>
      <h2 class="face-title inactive"></h2>
      <p class="face-description inactive"></p>
      <div class="action-button inactive"><p class="action-button-icon">&#x2913;</p></div>
    `;

    const canvas = container.querySelector('canvas');
    const titleEl = container.querySelector('.face-title');
    const descEl = container.querySelector('.face-description');
    const btnEl = container.querySelector('.action-button');
  ...
  window.createInfiniteMenu = createInfiniteMenu;
})();
